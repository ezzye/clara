...
export function BacklogView({
  ...
  return (
    <>
      ...
      {mode === "inventory" ? (
        <InventoryView state={state} act={act} busy={busy} onEdit={onEdit} />
      ) : (
        <>
          <div className="stats">
            <div className="card">
          ...
          <div className="card">
            ...
            <p>
              Your priority is one factor, alongside deadlines, preparation,
              project finishing and Clara’s recommendation. Available time,
              protected appointments, energy and the daily task limit decide
              what fits.
            </p>
            <p className="muted">
              This list is complete for tasks Clara currently knows about.
              Unconnected accounts and uncaptured commitments can still be
              missing.
            </p>
            {last ? (
              <p>
                Last planning pass: {new Date(last.at).toLocaleString("en-GB")}{" "}
                · {last.openCount} open tasks considered · {last.days} day
                window from {last.date}.
              </p>
            ) : (
              <p>
                No recorded planning pass yet. Use “Plan my day” to produce a
                trace of what was considered.
              </p>
            )}
          </div>
          ...
          {rows.map((row) => {
            ...
            return (
              <section className="card backlog-row" key={row.id}>
                ...
                <p className="muted">
                  {task.minutes} estimated minutes · {row.reservedMinutes}{" "}
                  reserved
